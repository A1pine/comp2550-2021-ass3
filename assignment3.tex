...
\par\noindent \newline After deciding our research topic, we read more paper in the following weeks because we lacked the basic knowledge of how neural architecture search works. There are many search algorithms for neural architecture search. For example, in "Neural architecture search with reinforcement learning" \cite{DBLP:journals/corr/ZophL16}, researchers implement neural architecture with reinforcement learning algorithms. In "Progressive Neural Architecture Search" \cite{DBLP:journals/corr/abs-1712-00559}, the Sequential Model-Based Optimization (SMBO) algorithm for NAS. Also, in "Deep Multimodal Neural Architecture Search" \cite{DBLP:journals/corr/abs-2004-12070}, a Neural SuperNet is used for searching for the best combination of operations, where operations are sub-neural network architectures. 

\par\noindent \newline There are pros and cons for each searching algorithm of NAS. In terms of using reinforcement learning in NAS, a reward (could be either positive or negative) is given for choosing any of the operation (sub-neural network). During training, the controller would try to maximize the expected reward. However, reinforcement learning is usually time-consuming and demands performant computational resources. It is shown in Table 4 in \cite{DBLP:journals/corr/abs-1808-05584} that it took 800 GPUs 28 days to train the NasNet. Therefore, we decided not to use this method. For "Progressive Neural Architecture Search" \cite{DBLP:journals/corr/abs-1712-00559}, it uses SMBO to implement neural architecture search. For SMBO, it begins with basic models first. It would only attempt complicated ones if required. Although using SMBO could avoid the NAS model being trapped in a complex model, this model is still time-consuming. It used 500 P100 GPUs and four days to train the Progressive Neural Architecture Search model. In terms of constructing a SuperNet and using gradient-descent for optimization, we focused on the MMnas paper \cite{DBLP:journals/corr/abs-2004-12070}. Usually, the controller is implemented with a recurrent neural network. RNNs generally could not be trained parallelly. Thus, the current NAS models with controllers are often slow (time-consuming). MMnas abandoned the meta-controller design and could speed up the neural architecture search process. Also, the MMnas focuses on figuring out the optimal transformer architectures. It is similar to our research topic. Therefore, we decided to intensively study the "Deep Multimodal Neural Architecture Search" \cite{DBLP:journals/corr/abs-2004-12070} paper.
